...
if __name__ == "__main__":
    pass
    # from adaptive_layer import TextAdaptiveLayer
    # import numpy as np

    # def prepare_cross_attention_mask(emb, cad_seq_len):
    #     emb = np.tile(emb, (cad_seq_len, 1))  # (512) -> (272, 512)
    #     emb = np.where(
    #         emb, -np.inf, 0
    ...
    # cad_data = torch.load(
    #     "/netscratch/ali/DeepCAD_Dataset/original/0011/00112915/seq/00112915.pth"
    # )
    # prompt_emb = dict(
    #     np.load(
    #         "/netscratch/ali/DeepCAD_Dataset/original/0011/00112915/prompt_embedding/mistral_sup/00112915_level_1_response.npz",
    #         allow_pickle=True,
    #     )
    # )

    # prompt_emb["attention_mask"] = np.pad(
    #     prompt_emb["attention_mask"],
    #     (0, 512 - len(prompt_emb["attention_mask"])),
    #     constant_values=1,
    # )

    # prompt_emb["embedding"] = np.pad(
    #     prompt_emb["embedding"],
    #     ((0, 512 - len(prompt_emb["embedding"])), (0, 0)),
    #     constant_values=0,
    # )

    # cad_vec = cad_data["vec"]
    # mask = cad_data["mask_cad_dict"]
    # ZE = torch.from_numpy(prompt_emb["embedding"]).unsqueeze(0).cuda()
    # ca_mask = {}
    # ca_mask["key_padding_mask"] = (
    #     torch.from_numpy(prompt_emb["attention_mask"] == 1).unsqueeze(dim=0).cuda()
    # )
    # ca_mask["attn_mask"] = (
    #     torch.from_numpy(
    #         prepare_cross_attention_mask(prompt_emb["attention_mask"], 271)
    #     )
    #     .unsqueeze(dim=0)
    #     .cuda()
    # )
    # for key, val in cad_vec.items():
    #     cad_vec[key] = val[:-1].unsqueeze(dim=0).cuda()

    # mask["attn_mask"] = mask["attn_mask"].unsqueeze(dim=0).cuda()
    # mask["key_padding_mask"] = mask["key_padding_mask"][:-1].unsqueeze(dim=0).cuda()

    # adaptive_layer = TextAdaptiveLayer(4096, 4096, 8, 0.1).cuda()

    # ZE_downsampled, attn_weight = adaptive_layer(
    #     ZE,
    #     {
    #         "attn_mask": None,
