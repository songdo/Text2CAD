...
class Text2CAD_Dataset(Dataset):
    def __init__(
        self, cad_seq_dir: str, prompt_path: str, split_filepath: str, subset: str, max_workers: int
    ):
        """
        Args:
        ...
        self.prompt_df=self.prompt_df[self.prompt_df['abstract'].notnull() & self.prompt_df['beginner'].notnull() & self.prompt_df['intermediate'].notnull() & self.prompt_df['expert'].notnull()]
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for data in tqdm(
                executor.map(self._prepare_data, self.uid_pair),
    ...
    def _prepare_data(self, uid):
        ...
        while not isinstance(prompt,str) or len(prompt_choices) < 4:
            prompt_choice = random.choice(list(set(self.all_prompt_choices) - prompt_choices))
